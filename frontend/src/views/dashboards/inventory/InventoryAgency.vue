...
import { ref, toRefs, reactive, h, onBeforeMount } from "vue"
...
onBeforeMount(async () => {
  ...
  const stores: any = await PayrollsModule.getAgenciesDropdownData({ user, isManagerRole })
  options.value = stores
})

const role: any = AuthModule.user?.role
const route : string = "/inventory/table?type=Agency"
