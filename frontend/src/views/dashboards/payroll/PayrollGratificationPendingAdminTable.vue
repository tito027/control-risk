...
async function onAccepted(payload: any) {
  try {
    ...
    const result = await PayrollsModule.updateGratificationStatus(body)
    if (result.update) {
      ElMessage({
        showClose: true,
  ...
  try {
    ...
    if (result.update) {
      ElMessage({
        showClose: true,
        message:
          "Se rechazó correctamente el gratificante pendiente a revisión",
        type: "success",
      })
      TablesModule.setReload(true)
    } else {
      ElMessage({
        showClose: true,
        message: "No se pudo actualizar el gratificante pendiente a revisión",
        type: "error",
      })
    }
  } catch (error: any) {
    ...
    <div class="mt-4 row">
      <div class="col-12">
        <div class="card">
          ...
          >
            <div>
              <h5 class="mb-0">Gratificantes pendientes a revisión</h5>
              <p class="mb-0 text-sm">
                A continuación se listan todos los gratificantes pendientes a
          ...
          <div class="table-responsive">
            <base-table
              ...
              endpoint="/payrolls/gratifications/pending/table/admin"
            >
