...
const onSubmitMasiveCreateDiscount = (payload: any) => {
  loading.value = true
  ...
  const users = !!users_roles
    ...
    : []
  const route = "payrolls/discounts/modals/massive/assign"
...
<template>
  <form-modal
    :show="show"
    title="Asignación masiva de descuento"
    modal-name="formCreateMasiveDiscountModal"
    column-classes="col-lg-6 col-md-6 col-12"
    :inputs="fields"
    :loading="loading"
    @change-modal="p => emits('changeModal', p.payload)"
    @submit="onSubmitMasiveCreateDiscount"
  />
</template>
