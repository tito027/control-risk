...
const fields: { [k: string]: Field } = {
  ...
  gratification_type: {
    ...
    url: "payrolls/gratifications/modals/types?is_scheduled=true",
    options: {
      label: "label",
      key: "value",
    },
...
const onSubmit = (payload: any) => {
  loading.value = true
  const route = "payrolls/gratifications/scheduled"
    ...
    Object.keys(props.state.payload).map(key => {
      ...
      if (validateKeys.includes(key)) {
        if (fields[key].type === "checkbox") {
          ...
          fields[key].value = moment(props.state.payload[key]).format(
            "dd/mm/yyyy"
          )
        } else {
...
<template>
  <form-modal
    ...
    :title="
      state.isEdit
        ? 'Editar gratificante recurrente'
        : 'Nuevo gratificante recurrente'
    "
    modal-name="formCreateScheduledGratificationModal"
